...
- `pms.auth_mode: okta_sso` consumes either the richer documented identity
  ...
  the Gmail/OTP verification fields are validated strictly. When that entire
  richer identity contract is absent, the Okta username is resolved from the
  platform-bound Gmail profile at login time. The deployed legacy
  `pms.okta_mfa: email` label is accepted as metadata, but automation proceeds
  only after detecting the exact Google Voice SMS controls in Okta; it does not
  implement an email-code fallback. This mode requires the platform-injected
  `MATON_API_KEY`. The browser opens Choice Connect, submits
...
- Outside explicitly authorized standalone direct-login testing, never bypass
  MFA. Production `okta_sso` completes only the exact Google Voice SMS factor
  detected in the live Okta UI; it does
...
be reused only when its displayed identity matches the configured or
gateway-resolved operations email. Refuse ambiguous frames, browser targets,
identities, Gmail mailboxes,
