...
def _reviewed_timestamp(value: str, property_local_date: date) -> str:
    """Validate the owner-local timestamp and its named IANA zone."""
    try:
        timestamp_text, zone_name = value.strip().rsplit(" ", 1)
...
def missing_reports_result(expected_features: Iterable[str]) -> dict[str, Any]:
    """Return the deterministic operator contract when no report was retrieved."""
    expected = list(expected_features)
    if not expected:
        expected = list(SUPPORTED_FEATURES)
    _validate_expected_features(expected)
    labels = " and ".join(FEATURE_LABELS[feature] for feature in expected)
    return {
        "status": "incomplete",
        "complete": False,
        "missing_features": expected,
        "next_question": (
            f"The {labels} report could not be retrieved. Should I retry once with "
            "fresh report parameters, or stop this audit?"
        ),
    }


def _validate_expected_features(features: list[str]) -> None:
    if not features:
    ...
    unsupported = [feature for feature in features if feature not in SUPPORTED_FEATURES]
    if unsupported:
    ...
    *,
    ...
    if missing_features:
        ...
        payload["next_question"] = (
            f"The {joined} report could not be included. Should I retry once with fresh "
            "report parameters, or deliver the available results as an incomplete audit?"
        )
    if ledger:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="requested audit feature; repeat for a full audit",
    )
    parser.add_argument("-o", "--out", required=True, help="audit_input.json destination")
    args = parser.parse_args()
    try:
        if not args.guest_ledger and not args.future_reservations:
            print(json.dumps(missing_reports_result(args.expected_feature or []), sort_keys=True))
            return 2
