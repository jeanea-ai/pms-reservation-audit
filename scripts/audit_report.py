...
def render_reportlab(spec, out_path):
    ...
    def build_flowables():
        ...
        if spec.get("disclaimer"):
            f.append(Paragraph(f'<font color="#1B6B6B">{_esc(spec["disclaimer"])}</font>', st_note))
        if spec.get("completion_warning"):
            f.append(Spacer(1, 3))
            f.append(Paragraph(f'<b><font color="#B00020">⚠ {_esc(spec["completion_warning"])}</font></b>', st_prop))
    ...
    def _footer(canvas, doc):
        ...
        canvas.drawString(MARGIN, 0.4 * inch, "Read-only audit — generated by Kolo")
...
def render_html_fallback(spec, out_path):
    ...
    for sec in spec.get("sections", []):
        ...
        for t in sec.get("tables", []):
            ...
            body = ""
            for r in t.get("rows", []):
                cells = "".join(f"<td>{esc(_cell_text(c))}</td>" for c in r)
                body += f"<tr>{cells}</tr>"
    ...
    warn = f"<div class='warn'>⚠ {esc(spec.get('completion_warning'))}</div>" if spec.get("completion_warning") else ""
...
h1 {{ color:#1F3A5F; font-size:20pt; margin:0 0 2pt; }}
h2 {{ color:#1B6B6B; font-size:13pt; border-bottom:1px solid #1B6B6B; margin:14pt 0 5pt; }}
p {{ margin:4pt 0; }}
.meta {{ margin:6pt 0; }}
.disclaimer {{ color:#1B6B6B; font-size:8.5pt; }}
.warn {{ color:#B00020; font-weight:bold; margin:4pt 0; }}
table {{ border-collapse:collapse; width:100%; margin:5pt 0; }}
th {{ background:#1F3A5F; color:#fff; font-size:8.5pt; text-align:left; padding:5pt; }}
...
.summary {{ font-weight:bold; color:#1F3A5F; margin-top:2pt; }}
